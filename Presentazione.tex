...
\begin{enumerate}
  ...
  \item Selezione N sequenze da un database (FluDB, NCBI)
  \item Partizionamento delle sequenze
  \item Calcolo matrice $d_{ij}$ delle $N(N-1)/2$ distanze tra partizioni
 \medskip{}
\uncover<2->{\item N punti, distanti tra di loro $d_{ij}$ --- grafo completo tra le sequenze\medskip{}}
\uncover<3->{\item Suddivisione di N sequenze in p \emph{blue}{clusters}\medskip{}}
\end{enumerate}
\begin{overprint}
 \begin{center}
 \includegraphics<4->[height=0.4\textheight,keepaspectratio=true]{presentazione-immagini/dendrogramma}
