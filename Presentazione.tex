...
\subsection{Rappresentazione partizioni}
...
\end{frame}
\subsection{Operazioni sulle partizioni}
\begin{frame}
...
\subsection{Distanza e riduzione}
\begin{frame}{Distanza di Rohlin}
...
H_\text{top} = \log(n) \qquad & \text{n \`e il numero di atomi} \\
			      & \small{\text{vedi Adler, Konheim, McAndrew (1965)}}
\end{align*}
...
\subsection{Virus dell'influenza}
...
\column{0.5\textwidth}
\includegraphics[height=0.45\textheight]{immagini/swine-flu}
%\center{\small{Struttura del virus}}
\end{columns}
\medskip{}
Sequenze a confronto:
\medskip{}
\includegraphics[width=0.95\textwidth]{presentazione-immagini/seq_lines2}
\end{frame}

\subsection{Applicazione distanze di Rohlin}
...
\begin{enumerate}
  ...
  \item<3-> Calcolo matrice $d_{ij}$ di $N(N-1)/2$ distanze tra partizioni
 \medskip{}
...
\end{frame}
% 
% \begin{frame}{Partizionamento alternativo}
% 
% \begin{exampleblock}{Partizioni con ``salto''}
% \begin{center}
% \textcolor{white}{
% \colorbox{blue}{AAAA} \colorbox{purple}{B} 
%  \colorbox{blue}{AAA} \colorbox{green!30!black}{DDDD} 
%  \colorbox{purple}{K} \colorbox{green!30!black}{DD} 
%  \colorbox{orange}{CCCCCC}
% }
% \end{center}
% % \begin{itemize}
% %  \item necessatiano di riduzione dicotomica
% %  \item tempo di calcolo estramemente pi\`u lungo
% %  \item scarsi vantaggi nel risultato
% % \end{itemize}
% 
% \end{exampleblock}
% 
%  \begin{block}{Alfabeti ridotti}
% \begin{center}
%  
%  \textcolor{white}{\colorbox{red}{LVIMC} \colorbox{black}{AG} 
%  \colorbox{orange}{ST} \colorbox{green!30!black}{P} 
%  \colorbox{purple}{FYW} \colorbox{blue}{EDNO} 
%  \colorbox{orange}{KR} \colorbox{black!70!white}{H}
% }
% \end{center}
%  \end{block}
% 
% 
% \center{\includegraphics[width=0.6\textwidth,keepaspectratio=true]{presentazione-immagini/clustering_fuzzy}}
% 
% \end{frame}

\section{Sistemi magnetici}

\subsection{Definizione hamiltoniana}
\begin{frame}{Sequenze lineari di spin (Ising 1D) }
Modello di sequenza aperta di spin, lunga L:
\[H=-\sum_{i}\, J_{i,i+1}\sigma_{i}\sigma_{i+1}\qquad i\in\left\{ 1,\dots,L\right\} \]

variabili di \emph{blue}{link}: \[l_{i}=\sigma_{i}\; \sigma_{i+1}\; \sgn (J_i)\qquad i\in\left\{ 1,\dots,L-1\right\}\]

gradi di libert\`a indipendenti: \[H=-\sum_{i} \left|J_i\right| l_{i}\]

generazione indipendente: \[p(l_i=+)=(1+e^{-2\beta |J_i|})^{-1}\quad \Longrightarrow\quad {\sigma_{i+1}}=l_{i}\,\sigma_{i} \sgn (J_i)\]

%\`e possibile tornare alla variabili originali \[l_{i}=\sigma_{i}\,\sigma_{i+1}\,\sgn (J_i)
%\[Z_{\text{sequenza}}=Z_{\text{link}}^{L}=\left(2\,\cosh\beta J\right)^{L}\]


\end{frame}

\subsection{Lunghezza di correlazione ``efficace''}
\begin{frame}
\frametitle{Lunghezza di correlazione tra partizioni}
Se J = cost > 0
\[
 \left\langle \sigma_{i}\,\sigma_{i+r}\right\rangle =\exp\left(-\frac{r}{\xi}\right)\qquad\text{con }\xi=-\frac{1}{\log\left(\tanh\beta J\right)}
\]

%

\bigskip{}
\emph{green!30!black}{Distanza di Rohlin}: sensibile solo ai bordi dei clusters
\bigskip{}

\center{\includegraphics[width=0.5\textwidth,keepaspectratio=true]{presentazione-immagini/spin_clusters}}
...
\begin{frame}
\frametitle{Variazione lunghezza}
il numero dei cluster $n \propto p\,L = n(\beta,L) \Longrightarrow$ entropia crescente in L!
\medskip{}
Maggiore complessit\`a delle partizioni $\Longrightarrow$ maggiore distanza media
\bigskip{}
\begin{columns}
 \column{0.4\textwidth}
\begin{block}{Estrazione lunghezza}
 \begin{enumerate}
  \item $\beta$ e L fissati
  \item generazione $J_i$
  \item generazione N sequenze a parametri fissati
  \item distanza media tra N sequenze
  \item media su J
  \item $L_{\xi}(\beta)=L$ a met\`a altezza
 \end{enumerate}

\end{block}

\column{0.6\textwidth}
\center{\includegraphics[width=1\textwidth,keepaspectratio=true]{ising/andamento_lunghezza}}
\end{columns}
\end{frame}


\begin{frame}{Dipendenza dalla temperatura}
\begin{overprint}
 
\only<1>{\center{\includegraphics[width=0.7\textwidth,keepaspectratio=true]{ising/confronto_temperature}}}

\only<2>{\center{\includegraphics[width=0.7\textwidth,keepaspectratio=true]{ising/ordinato_riscalamento}}}

\only<3>{\center{\includegraphics[width=0.7\textwidth,keepaspectratio=true]{ising/confronti_riscalato}}}

\end{overprint}

\only<2->{\begin{itemize}
 \item Corrispondenza perfetta
 \item Correzioni di reticolo finito
 \item Per $J=\pm 1$
\end{itemize}
}
...
quando J \`e disordinato ($J=\pm 1$)? $\left\langle \sigma_{i}\,\sigma_{i+r}\right\rangle = 0$
% {\small
% \begin{exampleblock}{Clusters di link}
%  \center{bordi dei cluster  $ \sigma_{i+1} = J_{i,i+1}\; \sigma_{i} \iff l_i = -1$}
% \end{exampleblock}}

\begin{itemize}
 \item se $J=\pm 1$ nessuna differenza, $L_{\xi}(\beta) \propto - (\log \tanh(\beta))^{-1}$
 \item se $J\in[a,b], a>0$: $L_{\xi}(\beta) \propto - (\log \tanh(\langle J \rangle\beta))^{-1}$
 \item se $J\in[-1,1]$, la possibilit\`a di $J=0$: $L_{\xi}(\beta) \propto \beta$
\end{itemize}

\center{\includegraphics[width=0.6\textwidth,keepaspectratio=true]{ising/confronto_j_rand_nonnull}}
...
\begin{itemize}
 \item $T > T_C$: distanza arriva a un massimo, si definisce $L_{\xi}(\beta)$
 \item $T < T_C$: \parbox[t]{0.8\textwidth}{la distanza diverge come $\mathcal{O}(\log(N))$ -- come l'integrale  \\
		 della correlazione sconnessa a due punti in 2D}
		  %$\left\langle \sigma_{i}\,\sigma_{i+r}\right\rangle_C + \left\langle \sigma_{i}\right\rangle
		  %  \,\left\langle\sigma_{i+r}\right\rangle$ }
\end{itemize}

\begin{center}
\includegraphics[width=0.7\textwidth,keepaspectratio=true]{ising/reticolo2d-3andamenti} 
\end{center}
...
Lavoro completato:
\begin{itemize}
 \item Ottimizzazione algoritmi di partizionamento e riduzione
 \item Generalizzazione a grafi e strutture generiche
 \item Nuovi metodi di riduzione
 \item Confronto e scelta delle distanze per sequenze dell'influenza
 \item Analisi completa di Ising 1D all'equilibrio anche nel caso disordinato
\end{itemize}
\bigskip{}
Work in progress:
\begin{itemize}
 \item Analisi della stagione influenzale 2011/2012
 \item Applicazioni a dinamiche conservate su grafi complessi
 \item Evoluzione temporale in fasi transienti per Ising 1D e 2D
 \item Calcolo lunghezza di correlazione in tutte le fasi per Ising 2D
\end{itemize}
