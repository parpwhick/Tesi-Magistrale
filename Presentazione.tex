...
Per una sequenza aperta di spin, lunga L
...
i gradi di libert\`a si disaccoppiano: \[H=-\sum_{i} J_i l_{i}\]

\`e possibile tornare alla variabili originali $l_{i}=\sigma_{i}\sigma_{i+1}\Longrightarrow {\sigma_{i}}=l_{i}\sigma_{i}$

\[Z_{\text{catena}}=Z_{\text{link}}^{L}=\left(2\,\cosh\beta J\right)^{L}\]
